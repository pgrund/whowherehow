...
const users = {
    ...
    hal: function halUser(user) {
        ...
        let session = sessions.all.find(
            s => s.teamMates.indexOf(user.playerId) > -1
        );
        let hal = {
            data: filtered,
            links: {
        ...
        switch (user.state) {
            ...
            case "PENDING":
                hal.links.approve = {
                    href: `/sessions/${session.sessionId}/players/${user.playerId}`,
                    title: "approve player for game"
                };
                hal.links.game = {
                    href: `/sessions/${session.sessionId}`,
                    name: session.sessionName
            ...
            case "INVITED":
                hal.links.approve = {
                    href: `/sessions/${session.sessionId}/players/${user.playerId}`,
                    title: "accept invitation for game"
                };
                hal.links.game = {
                    href: `/sessions/${session.sessionId}`,
                    name: session.sessionName
    ...
    update: function updateUser(addU, reqU) {
        ...
        users.all[
            users.all.findIndex(el => el.playerId === update.playerId)
        ] = update;
        return update;
    }
};
...
const sessions = {
    ...
    hal: function halSession(sess) {
        ...
        let hal = {
            ...
            links: {
                ...
                players: sess.teamMates
                    .map(id => users.all.find(p => p.playerId == id))
        ...
        if (sess.state == "OPEN") {
            hal.links.close = { href: `${url}/state`, title: "get session started" };
            hal.links.join = { href: `${url}/players`, title: "join session" };
        } else {
            hal.links.turn = {
                href: `${url}/turn`,
                title: "current turn of session"
            };
            hal.links.active = {
                href: `${hal.links.players[activePlayerIndex].href}`,
    ...
    update: function updateSession(addS, reqS) {
        ...
        sessions.all[
            sessions.all.findIndex(el => el.sessionId === update.sessionId)
        ] = update;
        return update;
    }
};
