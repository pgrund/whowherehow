...
function authenticatedUser(req, res, next) {
  ...
  if (!req.auth) {
    res.status(401).send("invalid user :" + req.cookies.privateId);
    return;
  }
  next();
}
...
router.post("/players", (req, res) => {
  ...
  if (user == undefined || user == null) {
    user = {
      ...
      name: req.body.name,
      playerId:
        Math.max(...storage.users.all.map(u => u.playerId).concat(0)) + 1
    };
    storage.users.all.push(user);
    user.privateId = user.playerId;
  } else {
    console.log(req.cookies.privateId, user.privateId);
